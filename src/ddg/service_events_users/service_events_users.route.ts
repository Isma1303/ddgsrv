...
import { ServiceEventsUsersController } from './service_events_users.controller'

const router = Router()
const endPoint = '/ddg/service-events-users'
const controller = new ServiceEventsUsersController()
...
router.delete('/deleteUserFromService', async (req: Request, res: Response) => {
    const response = await controller.deleteUserFromService(req)
    return res.status(response.statusCode).json(response)
})
...
const serviceEventsUsersRouter: Route = { endPoint, router }

export default serviceEventsUsersRouter
