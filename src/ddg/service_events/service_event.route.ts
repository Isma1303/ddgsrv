...
import { ServiceEventController } from './service_event.controller'

const router = Router()
const endPoint = '/ddg/service-event'
const controller = new ServiceEventController()
...
router.post('/send-reminder', async (req: Request, res: Response) => {
    const response = await controller.sendReminder(req)
    return res.status(response.statusCode).json(response)
})
...
const serviceEventRouter: Route = { endPoint, router }
