...
import OpcionMenuController from './menu_option.controller'
...
router.get('/getMenuOptions', async (req: Request, res: Response) => {
    const response = await controller.getMenuOptions(req)
    return res.status(response.statusCode).json(response)
})

router.get('/getNestedMenuOptions', async (req: Request, res: Response) => {
    const response = await controller.getNestedMenuOptions(req)
    return res.status(response.statusCode).json(response)
})
...
const menuOptionRouter: Ruta = { endPoint, router }
