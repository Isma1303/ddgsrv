...
} from "../../system/utils/cloudinary.util";
...
> {
  ...
  public async createProduct(req: Request): Promise<IResponse> {
    try {
      ...
      const result = await this.model.add(
        { ...newProduct, product_img: secure_url, public_id: public_id },
        userId,
      );
  ...
  public async updateProduct(req: Request): Promise<IResponse> {
    try {
      ...
      if (req.file) {
        const webpBuffer = await ensureWebp(
          req.file.buffer,
          req.file.mimetype,
          {
            quality: 82,
      ...
      const result = await this.model.updateById({
        id: productId,
  ...
  public async deleteProduct(req: Request): Promise<IResponse> {
    try {
      ...
      if (product.public_id) {
        await deleteFromCloudinary(product.public_id).catch((err) =>
